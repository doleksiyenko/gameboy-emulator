...
class CPU {
    ...
    private:
        ...
        // 16 bit registers
        uint16_t pc_ = 0x0; // program counter
        uint16_t sp_ = 0x0; // stack pointer
        uint16_t af_ = 0x0; // accumulator + flags
        uint16_t bc_ = 0x0; uint16_t de_ = 0x0; uint16_t hl_ = 0x0; // general-purpose registers (8 bit halves)
